...
void roundrobin()
{
	int c,k,i,j;
	c=time;
//round robin for queue3
	for(i=1;i<size_three;i++)
	{
		for(j=0;j<size_three-i;j++)
		{
			if(q_three[j].arrival_time>q_three[j+1].arrival_time)
			{
				swap(&q_three[j],&q_three[j+1]);
			}
		}
	}
	rem=size_three;
	while(rem!=0)
	{
		...
		{
			if(q_three[i].remaining_time>quant)
			{
			for(k=0;k<quant;k++)
			{
				time++;
				q_three[i].remaining_time--;
			 if(time-c==10)
             {
            	return;
			 }
			...
			if(q_three[i].remaining_time>0&&q_three[i].remaining_time<quant)
			{
				while(q_three[i].remaining_time!=0)
				{
				q_three[i].remaining_time--;
				time++;
			    if(time-c==10)
